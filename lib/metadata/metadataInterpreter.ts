...
function buildColumnRenderer<T>(renderConfig: any): (item: T) => React.ReactNode {
  if (!renderConfig) {
  ...
  const RenderComponent = function RenderColumn(item: any) {
    const key = renderConfig.key; // can override if needed
    // Resolve value from item
    const val = item[key] !== undefined ? item[key] : '';

    switch (type) {
      case 'text':
        return React.createElement('span', { className }, String(val));

      case 'text-fallback':
        return React.createElement('span', { className }, String(val || fallback));

      case 'badge':
        return React.createElement('span', { className }, String(val));

      case 'badge-fallback': {
        const resolvedVal = val || (fallbackKey ? item[fallbackKey] : fallback);
        return React.createElement('span', { className }, String(resolvedVal));
      }

      case 'badge-prefix':
        return React.createElement('span', { className }, `${prefix}${val}`);

      case 'fraction': {
        const num = item[numeratorKey] !== undefined ? item[numeratorKey] : 0;
        const den = item[denominatorKey] !== undefined ? item[denominatorKey] : 0;
        return React.createElement('span', { className }, `${num} / ${den}`);
      }
      ...
      case 'suffix':
        return React.createElement('span', { className }, `${val}${suffix}`);
      ...
      case 'image':
        ...
        return React.createElement('span', { className: fallbackClassName }, fallback);

      case 'boolean-badge': {
        const boolVal = Boolean(val === true || val === 'true');
        return React.createElement(
          'span',
          { className: boolVal ? trueClassName : falseClassName },
      ...
      case 'role-badge': {
        ...
        const isSuper = role.toLowerCase().includes('superadmin');
        ...
        return React.createElement(
          'span',
          { className: `inline-flex items-center px-2.5 py-1 rounded-lg text-[11px] font-bold border ${badgeColor}` },
          role
        );
      }
          ...
          choices.map((c: any, i: number) => {
            ...
            return React.createElement(
              ...
              React.createElement(
                ...
                React.createElement(
                  'span',
                  { className: 'inline-flex items-center justify-center w-5 h-5 rounded-md bg-indigo-50 text-indigo-700 font-bold text-[10px] border border-indigo-200 shrink-0 mt-0.5' },
                  letter
                ),
                React.createElement(
                  'span',
                  { className: 'text-slate-700 font-medium leading-relaxed break-words' },
                  c.text || '-'
                )
              ),
              React.createElement(
                'span',
                { className: 'inline-flex items-center px-2 py-0.5 rounded-md bg-slate-100 text-slate-700 font-bold text-[10px] border border-slate-200 shrink-0' },
                `Skor: ${score}`
              )
            );
          })
      ...
      case 'context-badges': {
        ...
        if (contexts.length === 0) {
          return React.createElement(
            'span',
            { className: 'inline-flex items-center px-2 py-0.5 rounded text-[10px] font-semibold bg-emerald-50 text-emerald-700 border border-emerald-100' },
            'Semua Konteks'
          );
        }
        ...
        return React.createElement(
          ...
          contexts.map((ctxKey, idx) => React.createElement(
            'span',
            {
              key: idx,
...
// Helper: Generic Columns Generator
function interpretColumns<T>(jsonColumns: any[]): ColumnMetadata<T>[] {
